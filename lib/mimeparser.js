 ...
 * 
 * @constructor
 * @param {String} rfc822 Raw body of the message
 */
...
/**
 * Processes header lines. Unfolds, splits lines to key-value pairs
...
Parser.prototype.processNodeHeader = function(){
    ...
    for(var i = this._node.header.length - 1; i >= 0; i--){
        if(i && this._node.header[i].match(/^\s/)){
            this._node.header[i-1] = this._node.header[i-1].replace(/\s*$/, "") + " " + this._node.header[i].trim();
            ...
            if(key in this._node.parsedHeader){
                ...
                this._node.parsedHeader[key] = value;
            }
        }
    }
