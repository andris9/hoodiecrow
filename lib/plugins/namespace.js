    ...
    server.setCommandHandler("NAMESPACE", function(connection, parsed, data, callback){
        ...
        if(parsed.attributes){
            connection.send({
                ...
                command: "BAD",
                attributes:[
                    {type: "TEXT", value: "Unexpected arguments to NAMESPACE"}
                ]
            }, "INVALID COMMAND", parsed, data);
            return callback();
        }
        ...
        Object.keys(server.namespace).forEach(function(key){
            var ns = server.namespace[key];
            if(list[ns.type]){
                list[ns.type].push([key + (key?ns.separator:""), ns.separator]);
            }
        });
