...
/**
 * Declares an ID plugin, see http://www.ietf.org/rfc/rfc2971.txt for details
 */
...
module.exports = function(server){
    ...
    server.setCommandHandler("ID", function(connection, parsed, data, callback){
        var clientList = {}, serverList = null, list, i, len, key;
        ...
        if(server.options.id){
            ...
            Object.keys(server.options.id).forEach(function(key){
                serverList.push({type:"STRING", value: key});
                serverList.push({type:"STRING", value: (server.options.id[key] || "").toString()});
            });
        }
        ...
        connection.send({
            ...
            command: "OK",
            attributes:[
                {type: "TEXT", value: "ID command completed"}
            ]
        }, "ID", parsed, data, clientList);
...
function sendError(message, connection, parsed, data, callback){
    ...
    }, "INVALID COMMAND", parsed, data, "ID");
    return callback();
}
