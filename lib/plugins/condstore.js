...
module.exports = function(server){
        ...
        if(changedsince){
            if(["ATOM", "STRING"].indexOf(changedsince.type) < 0 ||
                ...
                connection.send({
                    ...
                    command: "BAD",
                    attributes:[
                        {type: "TEXT", value: "Invalid syntax for CHANGEDSINCE, number expected"}
                    ]
        ...
        if(unchangedsince){
            if(["ATOM", "STRING"].indexOf(unchangedsince.type) < 0 ||
                ...
                connection.send({
                    ...
                    command: "BAD",
                    attributes:[
                        {type: "TEXT", value: "Invalid syntax for UNCHANGEDSINCE, number expected"}
                    ]
                }, "CONDSTORE FAILED", parsed, data);
                return callback();
            }
    ...
    server.storeFilters.push(function(connection, message, parsed){
        return parsed.unchangedsince >= message.MODSEQ;
    });

    server.outputHandlers.push(function(connection, response, description, parsed, data, message){
        if(!parsed){
            return;
        }

        // Increase modseq if flags are updated
        if(description == "FLAG UPDATE" && message){
