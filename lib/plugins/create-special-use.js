...
module.exports = function(server) {
    ...
    server.outputHandlers.push(function(connection, response, description, parsed) {
			// save it as a special use folder
        if (description === "CREATE" && parsed.mailboxSpecialUse && parsed.attributes && parsed.attributes.length > 0) {
					connection.setFolderSpecialUse(parsed.attributes[0].value,parsed.mailboxSpecialUse);
        }
    });
};