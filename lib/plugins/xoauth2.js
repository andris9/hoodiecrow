    ...
    server.setCommandHandler("AUTHENTICATE XOAUTH2", function(connection, parsed, data, callback) {
        ...
        if (parts.length !== 4 ||
            ...
            callback();
					...
					connection.server.users.authenticate({username: user, token: accessToken, method: 'XOAUTH2'},function (err) {
						if (err && err.type === "user") {
	            connection.send({
	                tag: parsed.tag,
	                command: "NO",
	                attributes: [{
	            ...
	            callback();
							...
							// do the xoauth2 authentication thing
