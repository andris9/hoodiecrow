...
module.exports = function(server){
    ...
    server.registerCapability("XOAUTH2", function(connection){
    ...
    server.setCommandHandler("AUTHENTICATE XOAUTH2", function(connection, parsed, data, callback){
        ...
        if(!server.capabilities["SASL-IR"] || !server.capabilities["SASL-IR"](connection)){
            connection.send({
                ...
                command: "BAD",
                attributes:[
                    {type: "TEXT", value: "SASL-IR must be enabled to support XOAUTH2"}
                ]
            }, "AUTHENTICATE XOAUTH2 FAILED", parsed, data);
            return callback();
        }
        ...
        if(parsed.attributes.length != 1 ||
            ...
            connection.send({
                ...
                command: "NO",
                attributes:[
                    {type: "TEXT", value: "Invalid SASL argument"}
                ]
            }, "AUTHENTICATE XOAUTH2 FAILED", parsed, data);
            return callback();
        }
        ...
        if(!connection.server.users.hasOwnProperty(user) ||
            ...
            connection.send({
                tag: "+",
                attributes:[
                    {type: "ATOM", value: new Buffer(JSON.stringify({
                        "status":"400",
                        "schemes":"Bearer",
            ...
            connection.inputHandler = function(){
                ...
                connection.send({
                    ...
                    command: "NO",
                    attributes:[
                        {type: "TEXT", value: "SASL authentication failed"}
                    ]
                }, "AUTHENTICATE XOAUTH2 FAILED", parsed, data);
            };
        }else{
