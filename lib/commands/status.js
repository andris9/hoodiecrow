...
module.exports = function(connection, parsed, data, callback){
    ...
    if(!parsed.attributes ||
        ...
        connection.send({
            ...
            command: "BAD",
            attributes:[
                {type: "TEXT", value: "STATUS expects mailbox argument and a list of status items"}
            ]
    ...
    var path = parsed.attributes[0].value,
        ...
        status, response = [],
        item, allowedStatus = ["MESSAGES", "RECENT", "UIDNEXT", "UIDVALIDITY", "UNSEEN"];
    ...
    for(var i=0, len = parsed.attributes[1].length; i<len; i++){
        ...
        if(!item || item.type != "ATOM" || allowedStatus.indexOf(item.value.toUpperCase()) < 0){
            connection.send({
                tag: parsed.tag,
                command: "BAD",
                attributes:[
                    {type: "TEXT", value: "Invalid status element (" + (i+1) + ")"}
                ]
            }, "STATUS FAILED", parsed, data);
            return callback();
        }

        response.push({type:"ATOM", value: item.value.toUpperCase()});
    ...
    connection.send({
        ...
        command: "OK",
        attributes:[
            {type: "TEXT", value: "Status completed"}
        ]
    }, "STATUS", parsed, data);
    return callback();
};
