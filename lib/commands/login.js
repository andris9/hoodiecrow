...
module.exports = function(connection, parsed, data, callback) {
    ...
    var users = connection.server.users,
		...
		// call the users.authenticate
		users.authenticate({username: username, password: password, method:'LOGIN'},function (err,data) {
			if (!!err) {
        connection.send({
            tag: parsed.tag,
            command: "NO",
            attributes: [{
				...
				connection.setState("Authenticated",username);
