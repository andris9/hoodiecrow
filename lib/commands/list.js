...
module.exports = function(connection, parsed, data, callback){
    ...
    if(!parsed.attributes ||
        ...
        connection.send({
            ...
            command: "BAD",
            attributes:[
                {type: "TEXT", value: "LIST expects 2 string arguments"}
            ]
    ...
    if(!parsed.attributes[1].value){
        ...
        var namespace = connection.server.namespace[parsed.attributes[1].value || ""];
        if(namespace){
            connection.send({
                tag: "*",
                command: "LIST",
                attributes:[
                    [{type:"ATOM", value: "\\Noselect"}],
                    namespace.separator,
                    ""
                ]
        ...
        folders.forEach(function(folder){
            connection.send({
                ...
                attributes:[
                    ...
                    connection.server.namespace[parsed.attributes[0].value].separator,
                    folder.path
                ]
