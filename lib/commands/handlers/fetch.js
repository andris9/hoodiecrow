...
fetchHandlers.UID = function(message) {
    return message.uid;
};

fetchHandlers.FLAGS = function(message) {
...
fetchHandlers.BODYSTRUCTURE = function(message) {
    if (!message.parsed) {
...
fetchHandlers.ENVELOPE = function(message) {
    if (!message.parsed) {
        message.parsed = mimeParser(message.raw);
    }
...
fetchHandlers["BODY.PEEK"] = function(message, query) {
    if (!query.section) {
        throw new Error("BODY.PEEK requires ans argument list");
    }
    return fetchHandlers.BODY(message, query);
};

fetchHandlers.BODY = function(message, query) {
...
fetchHandlers.INTERNALDATE = function(message) {
    return message.internaldate;
};

fetchHandlers.RFC822 = function(message) {
    return {
        type: "LITERAL",
        value: message.raw
    };
...
fetchHandlers["RFC822.SIZE"] = function(message) {
    return message.raw.length;
};

fetchHandlers["RFC822.HEADER"] = function(message) {
    if (!message.parsed) {
        message.parsed = mimeParser(message.raw);
    }
    return {
