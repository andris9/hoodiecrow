...
module.exports = function(connection, messageSource, params){
    ...
    Object.keys(connection.server.searchHandlers).forEach(function(key){
        ...
        if(connection.server.searchHandlers[key].params){
            queryParams[key] = connection.server.searchHandlers[key].params;
            return;
        }

        // if handler takes more than 3 params (mailbox, message, i), use the remaining as value params
        if(!(key in queryParams) && connection.server.searchHandlers[key].handler.length > 3){
            queryParams[key] = [];
            for(var i=0, len = connection.server.searchHandlers[key].handler.length - 3; i<len; i++){
        ...
        queryHandlers[key] = function(){
            ...
            messageSource.forEach(function(message, i){
                if(connection.server.searchHandlers[key].handler.apply(null, [connection, message, i + 1].concat(args))){
                    nrCache[message.uid] = i + 1;
                    results.push(message);
                }
            });
    ...
    return {list: totalResults, numbers: nrCache};
};