...
IMAPMockServer.addCommandHandler("SELECT", function(connection, tag, data, callback){
    ...
    connection.usedFlags
    connection.selectedMailbox.messages

    connection.send("*", "FLAGS (" + connection.options.permanentFlags.join(" ") + ")");
    connection.send("*", "OK [PERMANENTFLAGS (" + connection.options.permanentFlags.join(" ") + " \\*)  Flags permitted.");
...
IMAPMockServer.addCommandHandler("STATUS", function(connection, tag, data, callback){
    ...
    var mailboxName = (data.shift() || ""),
        mbox = mbox = connection.directoryCache[connection.server.checkMailboxName(mailboxName)] || false,
