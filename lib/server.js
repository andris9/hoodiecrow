...
IMAPMockServer.prototype.init = function(){
    ...
    this.options.flags = this.options.flags || ["\\Answered", "\\Flagged", "\\Draft", "\\Deleted", "\\Seen"];
                ...
                if(!param.value){
                    ...
                    if(!param.params.length){
                        ...
                        switch(param.params[0]){
                            case "HEADER":
                                param.responseName = paramName + "";
                                response["BODY[HEADER]"] = {_LITERAL: (message.structured.header || []).join("\r\n") + "\r\n\r\n"};
                                break;
                            case "HEADER.FIELDS":
                                if(!param.params[1]){
                                    // TODO: Erroneus situation
                                    param.responseName = paramName + "";
                                    response["BODY[HEADER.FIELDS]"] = {_LITERAL: (message.structured.header || []).join("\r\n") + "\r\n\r\n"};
                                }else{
...
IMAPMockServer.addCommandHandler("CREATE", function(tag, data, callback){
    ...
    for(var i=0, len = path.length; i<len; i++){
        ...
        if(!mbox){
            ...
            curBranch = {
                path: name,
                flags: [i < len - 1 ? "\\HasChildren" : "\\HasNoChildren"],
                uidvalitity: 1,
                uidnext: 1,
                messages: []
            }
