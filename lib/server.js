...
IMAPMockServer.prototype.createMailbox = function(mailboxName, oldMbox){
    ...
    if(mbox){
        if(mbox.flags.indexOf("\\NonExistent") < 0){
            ...
            for(var j = mbox.flags.length - 1; j >= 0; j--){
                if(
...
IMAPMockServer.addCommandHandler("CREATE", function(tag, data, callback){
    ...
    try{
        this.server.createMailbox(data.shift() || "");
    }catch(E){
        this.send(tag, E.message);
        return callback();
    }
...
IMAPMockServer.addCommandHandler("DELETE", function(tag, data, callback){
    ...
    try{
        this.server.deleteMailbox(data.shift() || "");
    }catch(E){
        this.send(tag, E.message);
        return callback();
    }
...
IMAPMockServer.addCommandHandler("RENAME", function(tag, data, callback){
    if(["Authenticated", "Selected"].indexOf(this.state) < 0){
        this.send(tag, "BAD Unknown command");
        return callback();
    }
    ...
    var oldMailboxName = (data.shift() || ""),
        ...
        oldMbox = this.directoryCache[this.server.checkMailboxName(oldMailboxName)] || false,
        newMbox = this.directoryCache[this.server.checkMailboxName(newMailboxName)] || false;

    if(!oldMbox || oldMbox.flags.indexOf("\\NonExistent") >= 0){
        this.send(tag, "NO [TRYCREATE] Unknown Mailbox: " + oldMailboxName + " (Failure)");
        return callback();
    }

    if(newMbox && newMbox.flags.indexOf("\\NonExistent") < 0){
        this.send(tag, "NO Mailbox " + newMailboxName + " exists (Failure)");
        return callback();
    }

    try{
        this.server.deleteMailbox(oldMailboxName);
    }catch(E){
        this.send(tag, E.message);
        return callback();
    }
    ...
    try{
        this.server.createMailbox(newMailboxName, oldMbox);
    }catch(E){
        this.send(tag, E.message);
        return callback();
    }
    ...
    this.processNotices();
    this.send(tag, "OK RENAME Completed");
    callback();
});

IMAPMockServer.addCommandHandler("SEARCH", function(tag, data, callback){
    if(this.state != "Selected"){
        this.send(tag, "BAD Unknown command");
        return callback();
    }
    ...
    var queryHandlers = {
        ...
        "ANSWERED": function(){
            ...
            this.selectedMailbox.messages.forEach(function(message, i){
                if(message.flags.indexOf("\\Answered") >=0 ){
                    nrCache[message.uid] = i + 1;
        ...
        "DRAFT": function(){
            var results = [];
            this.selectedMailbox.messages.forEach(function(message, i){
                if(message.flags.indexOf("\\Draft") >=0 ){
                    nrCache[message.uid] = i + 1;
                    results.push(message);
                }
            });
        ...
        "FLAGGED": function(){
            var results = [];
            this.selectedMailbox.messages.forEach(function(message, i){
                if(message.flags.indexOf("\\Flagged") >=0 ){
                    nrCache[message.uid] = i + 1;
                    results.push(message);
                }
            });
        ...
        "KEYWORD": function(keyword){
            var results = [];
            this.selectedMailbox.messages.forEach(function(message, i){
                if(message.flags.indexOf(keyword) >=0 ){
                    nrCache[message.uid] = i + 1;
                    results.push(message);
                }
            });
        ...
        "UNFLAGGED": function(){
            var results = [];
            this.selectedMailbox.messages.forEach(function(message, i){
                if(message.flags.indexOf("\\Unflagged") < 0){
                    nrCache[message.uid] = i + 1;
                    results.push(message);
                }
            });
    ...
    query.forEach((function(q, i){
        ...
        var currentResult = queryHandlers[q.shift()].apply(this, q);
    ...
    if(totalResults.length){
        ...
        this.send("*", "SEARCH " + (totalResults.map(function(message){
            return nrCache[message.uid]
        })).join(" "));
    }

    this.send(tag, "OK SEARCH completed (Success)");
...
function composeQuery(params){
    ...
    var queryParams = {
            "BCC": 1,       "BEFORE": 1,    "BODY": 1,      "CC": 1,
            "FROM": 1,      "HEADER": 2,    "KEYWORD": 1,   "LARGER": 1,
            "NOT": 1,       "ON": 1,        "OR": 2,        "SENTBEFORE": 1,
            "SENTON": 1,    "SENTSINCE": 1, "SINCE": 1,     "SMALLER": 1,
            "SUBJECT": 1,   "TEXT": 1,      "TO": 1,        "UID": 1,
            "UNKEYWORD": 1
        },
        pos = 0,
        param,
    ...
    var getParam = function(level){
        ...
        var param = params[pos++],
            paramCount = queryParams[param.toUpperCase()] || 0,
