...
IMAPMockConnection.prototype.buildSearchResponse = function(params, nrCache){
    var totalResults = [],
        ...
        queryHandlers = {
            ...
            "BODY": function(value){
                ...
                this.selectedMailbox.messages.forEach(function(message, i){
                    var body = (message.body || "").toString().split(/\r?\n\r?\n/);
                    body.shift();
                    body = body.join("\r\n\r\n");
                    if(body.toLowerCase().indexOf(value.toLowerCase()) >= 0){
                        nrCache[message.uid] = i + 1;
                        results.push(message);
                    }
                });
    ...
    if(params[0] && 
      !queryHandlers[(params[0] || "").toString().toUpperCase()] && 
      params[0].match(/^[a-z][a-z0-0\-\_:]+$/i)){
        // FIXME: charset is currently ignored
        charset = params.shift();
    }
...
IMAPMockConnection.prototype.buildMessageResponse = function(message, params, isUid){
    ...
    list.forEach((function(param){
        ...
        if(!this.mailboxReadOnly && (paramName == "RFC822" || param.value == "BODY")){
...
IMAPMockServer.addCommandHandler("STORE", function(tag, data, callback){
    ...
    if(this.mailboxReadOnly){
        this.send(tag, "NO Read only mailbox");
        return callback();
    }
...
IMAPMockServer.addCommandHandler("CLOSE", function(tag, data, callback){
    ...
    this.send(tag, "OK Returned to authenticated state");
    callback();
});
    ...
    switch(command){
        ...
        case "SEARCH":
            ...
            var params = [].concat(data || []),
                results = this.buildSearchResponse(params, {});
            ...
            if(results.length){
                ...
                this.send("*", "SEARCH " + (results.map(function(message){
                    return message.uid;
                })).join(" "));
            }
...
IMAPMockServer.addCommandHandler("SEARCH", function(tag, data, callback){
    ...
    var params = [].concat(data || []),
        nrCache = {},
        results = this.buildSearchResponse(params, nrCache);
