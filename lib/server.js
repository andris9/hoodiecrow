...
function IMAPMockServer(options){
    ...
    this.users = {};
    this.capabilities = {};
    this.commandHandlers = {}; // plugin defined commands
    this.messageInsertHandlers = [];
...
IMAPMockServer.prototype.init = function(){
    [].concat(this.options.enabled || []).forEach((function(capability){
        if(typeof capability == "function"){
            ...
            try{
                require("./plugins/" + capability.toLowerCase())(this);
            }catch(E){

                switch(capability){
                    case "LOGINDISABLED":
                        this.addCapability(capability, function(connection){
                            return connection.state == "Not Authenticated" && !connection.secureConnection;
                        });
                        break;
                    case "STARTTLS":
                        this.addCapability(capability, function(connection){
    ...
    var walkTree = (function(path, branch){
        Object.keys(branch).forEach((function(key){
            ...
            curBranch.messages.forEach((function(message, i){
                ...
                this.messageInsertHandlers.forEach((function(handler){
                    handler(curBranch, message, i + 1);
                }).bind(this));
            }).bind(this));
...
IMAPMockServer.prototype.addMessageInsertHandler = function(handler){
    this.messageInsertHandlers.push(handler);
};

IMAPMockServer.prototype.addMessageUpdateHandler = function(handler){
    this.messageUpdateHandlers.push(handler);
};

IMAPMockServer.prototype.addMailboxInfoHandler = function(handler){
    this.mailboxInfoHandlers.push(handler);
};
...
IMAPMockServer.prototype.setSearchHandler = function(keyword, params, handler){
    keyword = (keyword || "").toString().toUpperCase();
    if(!handler && typeof params == "function"){
        handler = params;
        params = undefined;
    }

    this.searchHandlers[keyword] = {
        keyword: keyword,
        params: params && [].concat(params || []) || false,
        handler: handler
    };
};
...
IMAPMockServer.prototype.updateFlags = function(mailbox, message, modifier, flags){
    ...
    this.messageUpdateHandlers.forEach(function(handler){
        handler(mailbox, message, modifier, flags);
    });
...
IMAPMockServer.prototype.addMessage = function(mailboxName, message, client){
    ...
    // ensure that internaldate is a Date object
    message.internaldate = message.internaldate ?
        (Object.prototype.toString.call(message.internaldate) == "[object Date]" ?
            message.internaldate : new Date(message.internaldate)) :
        new Date();
    ...
    this.messageInsertHandlers.forEach((function(handler){
        handler(mbox, message, mbox.messages.length);
    }).bind(this));
...
IMAPMockServer.prototype.addCommandHandler = function(command, handler){
    command = (command || "").trim().toLowerCase();
...
IMAPMockConnection.prototype.processCommand = function(data, callback){
    ...
    if(typeof this.server.commandHandlers[command] == "function"){
        // Custom commands
        this.server.commandHandlers[command](this, tag, data, callback);
    }else if(typeof IMAPMockServer.commandHandlers[command] == "function"){
        // Built in commands
...
IMAPMockConnection.prototype.checkSupport = function(capability){
    ...
    if(typeof this.server.capabilities[capability] == "function" &&
      this.server.capabilities[capability](this)){
        return true;
    }
    return false;
};
...
IMAPMockConnection.prototype.buildSearchResponse = function(params, nrCache){
    ...
    Object.keys(this.server.searchHandlers).forEach((function(key){

        // defined required params
        if(this.server.searchHandlers[key].params){
            queryParams[key] = this.server.searchHandlers[key].params;
            return;
        }

        // if handler takes more than 3 params (mailbox, message, i), use the remaining as value params
        if(!(key in queryParams) && this.server.searchHandlers[key].handler.length > 3){
            queryParams[key] = [];
            for(var i=0, len = this.server.searchHandlers[key].handler.length - 3; i<len; i++){
        ...
        queryHandlers[key] = function(){
            ...
            this.selectedMailbox.messages.forEach((function(message, i){
                if(this.server.searchHandlers[key].handler.apply(this, [this.selectedMailbox, message, i + 1].concat(args))){
                    nrCache[message.uid] = i + 1;
                    results.push(message);
                }
            }).bind(this));
...
IMAPMockConnection.prototype.buildFetchResponse = function(message, params, isUid){
    var response = {}, value;
...
IMAPMockConnection.prototype.buildMailboxInfo = function(connection, mailbox){
    ...
    list = [
        "FLAGS (" + flags.join(" ") + ")",
        "OK [PERMANENTFLAGS (" + flags.join(" ") + " \\*)] Flags permitted.",
        "OK [UIDVALIDITY " + mailbox.uidvalitity + "] UIDs valid.",
        mailbox.messages.length+" EXISTS",
        "0 RECENT",
        "OK [UIDNEXT " + mailbox.uidnext + "] Predicted next UID."
    ];

    this.server.mailboxInfoHandlers.forEach(function(handler){
        handler(mailbox, list);
    });
...
IMAPMockServer.addCommandHandler("FETCH", function(connection, tag, data, callback){
    ...
    connection.getMessageRange(range).forEach(function(item){
        connection.send("*", item[0] + " FETCH " + connection.buildFetchResponse(item[1], params));
    });
...
IMAPMockServer.addCommandHandler("STORE", function(connection, tag, data, callback){
    ...
    connection.getMessageRange(data.shift()).forEach(function(item){
        connection.server.updateFlags(connection.selectedMailbox, item[1], data[0], data[1]);
...
IMAPMockServer.addCommandHandler("SELECT", function(connection, tag, data, callback){
    ...
    connection.buildMailboxInfo(connection, mbox).forEach(function(line){
...
IMAPMockServer.addCommandHandler("STARTTLS", function(connection, tag, data, callback){
    ...
    var secureConnector = starttls(connection.socket, credentials, function(socket){
        ...
        if(!socket.authorized && connection.options.debug){
    ...
    switch(command){
        ...
        case "FETCH":
            ...
            connection.getMessageRange(range, true).forEach(function(item){
                connection.send("*", item[0] + " FETCH " + connection.buildFetchResponse(item[1], params, true));
            });
        ...
        case "STORE":
            ...
            connection.getMessageRange(range, true).forEach(function(item){
                connection.server.updateFlags(connection.selectedMailbox, item[1], data[0], data[1]);
                if((data[0] ||"").match(/^[\-+]?FLAGS$/i)){
                    connection.send("*", item[0] + " FETCH " + connection.buildFetchResponse(item[1], ["FLAGS"], true));
                }
            });
        ...
        case "SEARCH":
            ...
            if(results.list.length){
                ...
                connection.send("*", "SEARCH " + (results.list.map(function(message){
                    return message.uid;
                })).join(" "));
            }
