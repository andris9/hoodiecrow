...
IMAPConnection.prototype.createMessage = function (path,msg,callback) {
	this.userMailbox.createMessage(path,msg,callback);
};
...
IMAPConnection.prototype.setState = function (state,user) {
	...
	if (state === "Authenticated") {
		...
		this.userMailbox = this.server.storage.mailbox(user,user);
...
IMAPConnection.prototype.addFlags = function (messages,flags,cb) {
	messages = [].concat(messages || []);
	flags = [].concat(flags || []);
  flags = normalizeSystemFlags(flags);
  if (!checkSystemFlags(this.server.systemFlags, flags)) {return cb("Invalid system flags");}
	this.userMailbox.addFlags(this.selectedMailbox.path,messages,flags,cb);
};
IMAPConnection.prototype.removeFlags = function (messages,flags,cb) {
	messages = [].concat(messages || []);
	flags = [].concat(flags || []);
  flags = normalizeSystemFlags(flags);
  if (!checkSystemFlags(this.server.systemFlags, flags)) {return cb("Invalid system flags");}
	this.userMailbox.removeFlags(this.selectedMailbox.path,messages,flags,cb);
};
IMAPConnection.prototype.replaceFlags = function (messages,flags,cb) {
	messages = [].concat(messages || []);
	flags = [].concat(flags || []);
  flags = normalizeSystemFlags(flags);
  if (!checkSystemFlags(this.server.systemFlags, flags)) {return cb("Invalid system flags");}
	this.userMailbox.replaceFlags(this.selectedMailbox.path,messages,flags,cb);
};
