...
function IMAPServer(options){
    ...
    this.storage = this.options.storage || {"":{folders:{"INBOX":{}}}};
...
IMAPServer.prototype.getMailbox = function(path){
    ...
    if(path.toUpperCase() == "INBOX"){
        path = "INBOX";
    }else if(path.toUpperCase().indexOf("INBOX" + this.storage[""].separator) === 0){
        path = "INBOX" + path.substr(5);
    }
    ...
    folderPath.forEach((function(folderName){
        ...
        if(parent != storage){
            ...
            if(parent.flags.indexOf("\\NoSelect") >= 0){
                for(j = parent.flags.length-1; j >= 0; j--){
                    if(parent.flags[j] == "\\NoSelect"){
                        parent.flags.splice(j, 1);
                        break;
                    }
                }
            }

            // Remove \HasNoChildren from parent
            if(parent.flags.indexOf("\\HasNoChildren") >= 0){
                for(j = parent.flags.length-1; j >= 0; j--){
                    if(parent.flags[j] == "\\HasNoChildren"){
                        parent.flags.splice(j, 1);
                        break;
                    }
                }
            }
            if(parent.flags.indexOf("\\HasChildren") < 0){
...
IMAPServer.prototype.deleteMailbox = function(path, keepContents){
    ...
    if(!this.storage[namespace]){
        ...
        if(mailbox.folders && Object.keys(mailbox.folders).length && !keepContents){
            ...
            if(parent != storage &&
                parent.flags.indexOf("\\NoSelect") >= 0 &&
...
IMAPServer.prototype.indexFolders = function(){
    ...
    Object.keys(this.storage).forEach((function(key){
        ...
        this.storage[key].separator = this.storage[key].separator || "/";
...
IMAPServer.prototype.processMailbox = function(path, mailbox, namespace){
    ...
    if(mailbox.folders && Object.keys(mailbox.folders).length){
        if(mailbox.flags.indexOf("\\HasChildren") < 0){
            mailbox.flags.push("\\HasChildren");
        }
        if(mailbox.flags.indexOf("\\HasNoChildren") >= 0){
            for(i = mailbox.flags.length - 1; i >= 0; i--){
                if(mailbox.flags[i] == "\\HasNoChildren"){
                    mailbox.flags.splice(i, 1);
                }
            }
        }
    }else{
        if(mailbox.flags.indexOf("\\HasNoChildren") < 0){
            mailbox.flags.push("\\HasNoChildren");
        }
        if(mailbox.flags.indexOf("\\HasChildren") >= 0){
            for(i = mailbox.flags.length - 1; i >= 0; i--){
                if(mailbox.flags[i] == "\\HasChildren"){
                    mailbox.flags.splice(i, 1);
                }
            }
        }
