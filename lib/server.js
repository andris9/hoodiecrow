 ...
 * @param {String} path Pathname for the mailbox
 */
IMAPServer.prototype.createMailbox = function(path){
    // Ensure case insensitive INBOX
    if(path.toUpperCase() == "INBOX"){
        path = "INBOX";
    }else if(path.toUpperCase().indexOf("INBOX" + this.storage[""].separator) === 0){
 ...
 * @param {String} path Pathname for the mailbox
 */
IMAPServer.prototype.deleteMailbox = function(path){
    // Ensure case insensitive INBOX
    if(path.toUpperCase() == "INBOX"){
        path = "INBOX";
    }else if(path.toUpperCase().indexOf("INBOX" + this.storage[""].separator) === 0){
    ...
    if(!this.storage[namespace]){
        ...
        if(mailbox.folders && Object.keys(mailbox.folders).length){
            ...
            // `reference` object. any new select is going to get `folder`
