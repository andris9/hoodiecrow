...
function IMAPServer(options) {
    ...
    if (this.options.ssl) {
		...
		if (this.options.tls && !this.options.ssl) {
			require("./plugins/starttls")(this);
		}
...
function IMAPConnection(server, socket) {
    ...
    this.secureConnection = !!this.options.ssl;
...
IMAPConnection.prototype.unsubscribeFolder = function (path,callback) {
  this.userMailbox.unsubscribeFolder(path,callback);
};
IMAPConnection.prototype.getStatus = function (path,callback) {
  this.userMailbox.getStatus(path,callback);
};
...
IMAPConnection.prototype.search = function (query,cb) {
	this.userMailbox.searchMessages(this.selectedMailbox.path,query,cb);
};
