...
function getBasicFields(node, options){
    var bodyType = node.parsedHeader['content-type'] && node.parsedHeader['content-type'].type || null,
        ...
        contentTransfer = node.parsedHeader['content-transfer-encoding'] || null;
    ...
    return [
        ...
        // body encoding
        options.upperCaseKeys ? contentTransfer && contentTransfer.toUpperCase() || null : contentTransfer,
        // body size
        node.size
    ];
}
...
function processTextNode(node, options){
    ...
    if(options.body){
        ...
        return data.concat(getExtensionFields(node, options));
    }
}
